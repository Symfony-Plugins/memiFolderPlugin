...
/**
 * Este método nos permite filtrar los campos que se presenta al usuario para la parte de compartir a grupos
 *
 * @param clase que mapea una tabla en la base de datos $object
 * @param (no es relevante) nombre del metodo que se usará aunque es solo un identificador $method
 * @param opciones que se envian como los nombres de las etiquetas, etc $options
 * @return un arreglo de obj_concretos
 */
function _get_grupos_compartir($object, $method, $options){
	
        $criteria_method= _get_option($options, 'filter');
        $criteria= call_user_func(array($object, $criteria_method));
        $through_class = _get_option($options, 'through_class');
...
/**
 * Este metodo devuelve los usuarios de los grupos a los que el usuario esta asociado
 *
 * @param el atributo $object hace referencia al objeto en Propel para el cual se realizará el mapeo N:M
 * @param $method define el nombre del metodo que se define para la consulta
 * @param $options define las opciones que se envian al double_list 
 * @return un arreglo que tiene al objeto y a sus obejtos asociados y lo más importante los ids de los objetos relacionados.
 */
function _get_usuarios_compartir($object, $method, $options){
    	$criteria_method= _get_option($options, 'filter');
        $criteria= call_user_func(array($object, $criteria_method));
        $through_class = _get_option($options, 'through_class');
        $objects = sfPropelManyToMany::getAllObjects($object, $through_class, $criteria);
